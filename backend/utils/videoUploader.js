...
const storage = multer.diskStorage({
    ...
    filename: async (req, file, cb) => {
        ...
        if (req.body.episodeNumber) {
            ...
            cb(null, `${title}-Season${seasonNumber}Episode${episodeNumber}-${shortid.generate()}-playcast${ext}`);
        }
        else {
            const title = req.body.title.replace(/\s+/g, '-');
            cb(null, `${title}-${req.body.release_date}-${shortid.generate()}-playcast${ext}`);
        }
    },
});
